...
const StudentDetail = () => {
        ...
        try {
            ...
            if (selectedFormation) {
                try {
                    ...
                    showAlert(t('student.updateSuccessAssignError'));
                }
            }
            ...
            if (!selectedFormation) showSuccess(t('student.updateSuccess'));
        } catch (error) {
            console.error('Error updating student:', error);
            showError(t('student.updateError'));
        } finally {
            setUpdating(false);
        }
    };
    ...
    if (!student) {
        return <div className="empty-state">{t('student.notFound')}</div>;
    }
    ...
    return (
        <div className="student-detail">
            ...
            <div className="detail-grid">
                <Card>
                    <h3>{t('student.personalInfo')}</h3>
                    <div className="info-grid">
                        <div className="info-item">
                            <div className="info-label">{t('auth.lastName')}</div>
                            <div className="info-value">{student.nom}</div>
                        </div>
                        <div className="info-item">
                            <div className="info-label">{t('auth.firstName')}</div>
                        ...
                        <div className="info-item">
                            <div className="info-label">{t('auth.dob')}</div>
                        ...
                        {student.adresse && (
                            <div className="info-item">
                                <div className="info-label">{t('auth.address')}</div>
                    ...
                    {studentFormations.length > 0 ? (
                        <div className="formations-list" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {studentFormations.map((f) => {
                                ...
                                const pct = st ? Math.min(100, Math.max(0, st.pourcentage_presence || 0)) : 0;
                                const present = st ? (st.present || 0) : 0;
                                const absent = st ? (st.absent || 0) : 0;
                                const total = st ? (st.total_seances || 0) : 0;
                                ...
                                return (
                                    <div key={f._id} style={{
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <h4 style={{ margin: 0 }}>{f.formation_id?.nom || t('formation.formation')}</h4>
                                            <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>
                                                {t('formation.level')}: {f.niveau_actuel} | {t('formation.status')}: {f.statut}
                                            </p>
                                            ...
                                            {st && (
                                                <div style={{ marginTop: '8px' }}>
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginTop: '6px' }}>
                                                        <span>{t('student.progress')}: {pct}%</span>
                                                        <span>{t('student.present')}: {present}/{total}</span>
                                                        <span>{t('student.absent')}: {absent}/{total}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                ...
                <form onSubmit={handleUpdateStudent}>
                    ...
                    {isResponsable && (
                        <div className="input-group">
                            <label className="input-label">{t('student.assignNewFormation')}</label>
                            <select
                                className="input"
                                ...
                                onChange={handleEditChange}
                            >
                                <option value="">{t('student.noAssignment')}</option>
                                {allFormations
